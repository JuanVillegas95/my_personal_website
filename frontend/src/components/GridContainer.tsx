...
import { GALLERY_PAGE_COUNT, GALLERY_SECTIONS, type GalleryItem } from './galleryData'
...
function MediaSurface({ item }: { item: GalleryItem }) {
  ...
  if (hasMedia && item.mediaType === 'video') {
    return (
      <video
  ...
  return (
    <div
      ...
      data-motion={item.mediaType === 'gif' || item.mediaType === 'video' ? 'true' : 'false'}
      aria-hidden="true"
    >
